angular.module('mngApp.facilityMngTempHum', ['ngRoute'])
.controller('facilityMngTempHum_ctrl', ['$scope', '$rootScope', '$http', '$timeout', '$q', '$route', 'itemNumList', 'guardDeviceStateList', 'RememberSer', 'TblPagination', function($scope, $rootScope, $http, $timeout, $q, $route, itemNumList, guardDeviceStateList, RememberSer, TblPagination) {
  $rootScope.globalPath.initPath({
    'name': '温湿度设备',
    'url': '../../..' + window.location.pathname + '#/facilityMng_tempHum'
  }, 'LV1');
  ...
  $scope.tblToolbar = {
    ...
    whenValChanged: function() {
      ...
      //   };
      // }
      $scope.tblNormal.getTempHumList(self.cityVal.id, self.shopVal.id, self.itemNumVal.id, 1, self.searchVal);
    },
  ...
  $scope.tblNormal = {
    getTempHumList: function(cityID, shopID, pageSize, curPage, key) {
      var self = this;
      $http.get('http://' + $rootScope.globalURL.hostURL + '/api/getTHListOfDeviceBKMgr?cityID=' + cityID + '&shopID=' + shopID + '&pageSize=' + pageSize + '&curPage=' + curPage + '&key=' + key)
        .success(function(ret) {
          console.log(ret);
  ...
  $scope.tblPagination.hookAfterChangePage = function(pageNum) {
    ...
    $scope.tblNormal.getTempHumList(tblToolbar.cityVal.id, tblToolbar.shopVal.id, self.pageSize, pageNum, tblToolbar.searchVal);
  };
  $q.all([$scope.tblToolbar.getCityList(),$scope.tblToolbar.getShopList('')])
  .then(function(){
    if (RememberSer.restore($scope)) {
      $scope.tblNormal.getTempHumList($scope.tblToolbar.cityVal.id, $scope.tblToolbar.shopVal.id, $scope.tblToolbar.itemNumVal.id, $scope.tblPagination.curPage, $scope.tblToolbar.searchVal);
    } else {
      $scope.tblNormal.getTempHumList('', '', $scope.tblToolbar.itemNumVal.id, 1, '');
    }
  })
}])
