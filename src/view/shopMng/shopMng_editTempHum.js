angular.module('mngApp.editTempHum', ['ngRoute'])
.controller('shopMngEditTempHum_ctrl',['$rootScope','$scope','$routeParams','$http','$timeout',function($rootScope,$scope,$routeParams,$http,$timeout){
  $scope.viewTag = 'Edit';
  ...
  $scope.tblDetails = {
    getTempHumDetails: function(id){
      $http.get('http://' + $rootScope.globalURL.hostURL + '/api/getTemperatureHumidityDetailBKMgr?id=' + id)
    ...
    save: function(){
      ...
      if(self.validateForm()){
        $http.get('http://' + $rootScope.globalURL.hostURL + '/api/updateTemperatureHumidityBKMgr?id=' + $scope.formResult.id + '&deviceName=' + $scope.formResult.deviceName + '&deviceSN=' + $scope.formResult.deviceSN + '&shopID='+$scope.shopID)
        .success(function(ret) {
          console.log(ret);
          $scope.modalBasic.header.content = '提示';
          if (ret.success) {
            //如果添加仓位成功
            $scope.modalBasic.body.content = '编辑温湿度设备成功！';
            ...
            //添加仓位失败
            $scope.modalBasic.body.content = '编辑温湿度设备失败！' + ret.message;
  ...
  $scope.tblDetails.getTempHumDetails($scope.id);
}])
