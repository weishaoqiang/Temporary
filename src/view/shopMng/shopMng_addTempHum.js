angular.module('mngApp.addTempHum', ['ngRoute'])
.controller('shopMngAddTempHum_ctrl',['$rootScope','$scope','$routeParams','$http','$timeout',function($rootScope,$scope,$routeParams,$http,$timeout){
  ...
  $scope.tblDetails = {
    ...
    save: function(){
      ...
      if(self.validateForm()){
        $http.get('http://' + $rootScope.globalURL.hostURL + '/api/addTemperatureHumidityBKMgr?shopID=' + $scope.formResult.shopID + '&deviceName=' + $scope.formResult.deviceName + '&deviceSN=' + $scope.formResult.deviceSN)
        .success(function(ret) {
          console.log(ret);
          $scope.modalBasic.header.content = '提示';
          if (ret.success) {
            //如果添加仓位成功
            $scope.modalBasic.body.content = '添加温湿度设备成功！';
            ...
            //添加仓位失败
            $scope.modalBasic.body.content = '添加温湿度设备失败！' + ret.message;
