...
Dates.getPath = (function(){
    ...
    //console.log(jsPath); //该文件的当前路径
    return config.path ? config.path : jsPath.substring(0, jsPath.lastIndexOf("/") + 1);
}());

Dates.use = function(path, id){
    var link = doc[creat]('link');
    link.type = 'text/css';
    link.rel = 'stylesheet';
    link.href = Dates.getPath + path ;
    console.log(Dates.getPath);
    id && (link.id = id);
...
//Dates.use('need');                         // Dates.getPath/need/laydate.css
//Dates.use(as[4] + config.defSkin, as[3]);  // Dates.getPath/skins/default/laydate.css, id=LayDateSkin
//Dates.skinLink = Dates.query('#'+as[3]);   //不管
...
//};
//Added By Ironmax------------------------------
Dates.useDirectlly = function(fileName, id){
    var js = document.scripts, jsPath = js[js.length - 1].src;
...
Dates.orien = function(obj, pos){
    var objComputedStyle = getComputedStyle(obj);
    var objWidth = parseInt(objComputedStyle.width);
    var tops, rect = Dates.elem.getBoundingClientRect();
    var elemRight = rect.right;
    var left = rect.left + (pos ? 0 : Dates.scroll(1));
    var winWidth = document.documentElement.clientWidth;
    var delta = objWidth+left-winWidth;
    // obj.style.left = rect.left + (pos ? 0 : Dates.scroll(1)) + 'px';
    obj.style.left = (delta>0)?(elemRight-objWidth+'px'):(left+'px');
...
Dates.init = (function(){
    ...
    //--First Modified
    //Dates.use('laydate.css');
    //Dates.use('skins/default/laydate.css', as[3]);
    //--Second Modified
    //Dates.useDirectlly('laydate.css');
    //Dates.useDirectlly('laydateSkin.css', 'LayDateSkin');
